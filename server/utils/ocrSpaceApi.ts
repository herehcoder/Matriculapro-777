...
// Interfaces para os resultados da análise da API OCR.space
...
// Interfaces para os resultados da análise (mantidas iguais à implementação anterior)
...
/**
 * Analisa um documento de identificação usando a API OCR.space
...
export async function analyzeDocument(
  ...
  try {
    ...
    // Preparar dados para envio à API OCR.space
    ...
    formData.append('OCREngine', '2'); // Usar o motor mais avançado (2)

    // Enviar requisição para API OCR.space
    ...
    if (response.data.OCRExitCode !== 1 || response.data.IsErroredOnProcessing) {
      return {
        success: false,
        error: response.data.ErrorMessage || 'Erro na análise OCR'
      };
    }
    ...
    // Extrair confiança média (se disponível)
    const confidence = response.data.ParsedResults[0]?.Confidence || 0;
    ...
    if (formFields && Object.keys(formFields).length > 0) {
      ...
      if (result.data) {
        result.data.validationResults = validationResults;
      }
    }
    ...
    console.error('Erro na análise do documento com OCR.space:', error);
    return {
      success: false,
